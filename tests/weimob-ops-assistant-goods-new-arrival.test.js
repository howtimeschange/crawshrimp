...
test('main searches Weimob goods one code at a time and prefers online-only duplicate', async () => {
  const calls = []
  ...
  assert.equal(result.meta.shared.products[1].goodsId, 154550689297432)
  assert.equal(result.meta.shared.products[1].saleChannelType, 1)
})
