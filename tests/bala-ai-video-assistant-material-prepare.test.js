...
test('buildCodePlan scans every top-ranked selected model folder', async () => {
  ...
  assert.deepEqual(Array.from(modelRows, row => row['云盘Hash']), ['second-ai', 'first-ai'])
  assert.deepEqual(Array.from(modelRows, row => row.__cloud_filehash), ['second-ai', 'first-ai'])
...
test('buildCodePlan selects model and detail folders then builds organized download queue', async () => {
  ...
  const keptModelRow = plan.rows.find(row => row['素材来源'] === '模拍图' && row['下载结果'] === '')
  assert.equal(keptModelRow['压缩结果'], '待压缩')
  assert.equal(keptModelRow['云盘Hash'], 'model-hash')
  assert.equal(keptModelRow.__cloud_filehash, 'model-hash')
})
