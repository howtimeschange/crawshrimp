...
- AI 生图脚本已有“云盘找素材 - GPT Image 生成 - 下载结果 - 审批/重试”的分阶段模型；换脸/换背景阶段应复用这套抓虾内置能力。软件管家智图/大森 AI 不再作为本期必接 provider，只作为人工备用或后续可选接口。
...
本方案的 AI 换脸/换背景阶段明确采用抓虾内置 GPT Image 生图能力，不再把软件管家智图或大森 AI 作为必接接口。软件管家智图/大森 AI 只保留为人工备用通道或后续可选 provider。这样做是方案取舍：童装图像生产要复用抓虾已有 AI 生图、任务创建、异步提交、结果缓存、审批与重试能力，把“换脸”和“换背景”统一成可审计的 AI 图片编辑任务，而不是再维护一条额外页面 RPA。
...
5. 为 Seedance 隐私拦截、API key 缺失、软件管家上传/轮询/下载失败、视频 URL 过期等真实坏天气补产品化文案和恢复路径。
...
3. `ai_model_swap_batch`
   ...
   - 输出：换脸图、失败原因、模型选择信息。
   - 安全边界：只允许授权 AI 模特库/品牌授权素材替换；不做指定真人身份换脸；儿童/童模素材必须保留人工审批。软件管家智图/大森 AI 仅作为备用 provider。
...
4. `qn_img2video_batch`
   ...
   - 模板选择：支持导出软件管家童装模板目录，提前下载模板示意视频；业务可按模板 ID 或关键词选择模板。
...
- 软件管家智图和大森 AI 若未来作为备用 provider，再确认登录态、额度和接口稳定性；本期 GPT Image 方案不依赖它们。
