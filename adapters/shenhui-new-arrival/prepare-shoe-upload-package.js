;(async () => {
  ...
  function inferShoeColorCode(fullpath, inputCode) {
    const segments = pathSegments(fullpath)
    const codeIndex = segments.findIndex(segment => pathContainsCode(segment, inputCode))
    if (codeIndex >= 0) {
      const color = segments.slice(codeIndex + 1).find(segment => /^\d{5}$/.test(segment))
      if (color) return color
    }
    return [...segments].reverse().find(segment => /^\d{5}$/.test(segment)) || ''
  }
  ...
  function classifyShoeAsset(item) {
    ...
    return {
      ...
      action: '下载后识别鞋品姿势',
      reason: '',
      packageFilename: toSafeFilename(item?.filename || `shoe.${ext}`, `shoe.${ext || 'jpg'}`),
    }
  }
  ...
  function rowForAsset(inputCode, sourceType, item, classification, overrides = {}) {
    ...
    const colorCode = inferShoeColorCode(item?.fullpath || '', inputCode)
    return {
      '输入款号': groupCode,
      '颜色': colorCode,
      ...
      '__shoe_original_filename': String(item?.filename || ''),
  ...
  async function buildSourcePlan(inputCode, sourceType, sourceConfig, codeIndex, totalCodes, options = {}) {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      const classification = classifyShoeAsset(item)
  ...
  async function buildCodePlan(inputCode, sourceConfigs, codeIndex, totalCodes, options = {}) {
    ...
    const sourceTypes = Array.isArray(options.sourceTypes) && options.sourceTypes.length
      ? options.sourceTypes.filter(sourceType => sourceType === 'shoe')
      : ['shoe']
  ...
  try {
    if (phase === 'init' || phase === 'main') {
      ...
      const shoeMount = await mountForPath(shoePath)
      const sourceConfigs = {
        shoe: {
          mountId: shoeMount.mountId,
          mountName: shoeMount.mountName,
          cloudPath: shoePath.raw,
          relativePath: shoePath.relativePath,
          broadRelativePath: deriveBroadSourcePrefix(shoePath.relativePath, 'shoe'),
        },
      }
    ...
    if (phase === 'collect_code') {
      ...
      const baseShared = {
        ...
        current_buyer_id: currentCode,
        current_store: `深绘鞋品上新图包整理 / ${getGroupCode(currentCode)}`,
        search_total_codes: codes.length,
