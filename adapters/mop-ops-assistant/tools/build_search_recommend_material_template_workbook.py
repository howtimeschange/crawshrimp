...
def build_workbook(output: Path) -> None:
    ...
    task_headers = ["商品ID", "素材图片", "素材张数", "添加标题", "内容描述", "裁剪比例", "备注"]
    ...
    ws_task.append([
        ...
        "3:4",
        "示例：素材图片可留空，在 UI 选择素材根目录后读取 商品ID/01.jpg 到 03.jpg",
    ])
    ws_task.append([
        "776047586897",
    ...
    for idx, width in enumerate([18, 72, 12, 24, 80, 12, 48, 88], 1):
    ...
    guide_rows = [
        ...
        ("素材张数", "否", "素材图片为空且填写了“素材根目录”时生效；搜推图文至少 3 张、最多 9 张", "3"),
        ...
        ("素材包摆放", "建议", "使用素材根目录时按“素材根目录/商品ID/01.jpg、02.jpg、03.jpg”摆放；扩展名固定为 jpg；png/jpeg/webp 请写入素材图片列或手动多选", "/Users/you/Desktop/搜推素材/1051467606993/01.jpg"),
        ("手动多选命名", "建议", "使用“手动选择素材图片”时，文件名以商品ID开头或父文件夹为商品ID；脚本会按商品ID自动归组", "1051467606993_01.jpg 或 1051467606993/01.jpg"),
    ]
