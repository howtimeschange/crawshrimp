const logFs=require('node:fs');console.log=(...args)=>logFs.appendFileSync(require('node:path').join(__dirname,'final-ui-windows.log'),args.join(' ')+'\n');console.error=(...args)=>logFs.appendFileSync(require('node:path').join(__dirname,'final-ui-windows.log'),args.map(String).join(' ')+'\n');
