...
- `v1.2.1`：Windows 首装/冷启动场景补齐后端 ready 等待、启动超时放宽和真实错误透传，`desktop-latest` 同步输出 `crawshrimp-v1.2.1-*` 安装包。
- `v1.2.0`：新增 `shein-helper` 首版适配包，Temu `后台-商品流量-列表` 与 JS 执行器分页恢复链路同步补强。
