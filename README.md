...
## v2.2.2 AI 视频工作流可靠性修复（2026-07-20）

`v2.2.2` 是一次补丁发布，修复 AI 视频工作流在真实工作区中的素材回显、任务状态与历史清理问题，并让 README 完整呈现当前 AI 生视频能力。

- 素材回显与一次性云盘下载款号解耦，只展示工作区中已经下载的素材；未选择工作区时提供明确提示，进入 AI 改图时默认展开全部款号。
- 任务列表和新增视频任务重新稳定显示本地原图、AI 图缩略图；本地预览失败时再回退远程地址，放大预览层级高于任务弹窗。
- 失败结果会结束加载态；清除失败任务会同时停止轮询并写入工作区清单，刷新页面后不会重新出现。
- 工作区内图片继续经过文件类型、普通文件和目录边界校验；通用图片预览不再受旧目录白名单限制。

完整变更见 [v2.2.2 Release Notes](release-notes/v2.2.2.md)。
