...
## 上上次更新（2026-06-12 / v1.4.27）

- 桌面端版本升级到 `v1.4.27`，正式发布会输出 `crawshrimp-v1.4.27-*` 命名的桌面安装包。
- MOP「批量KOL素材转短视频」和「搜推素材」支持同一款号下多个达人主图包自动拆成多条任务记录。
- 当导入表格里存在重复商家编码时，会按行顺序分配对应达人图片包，并保留每行不同的标题、描述、比例、提示词和备注配置。
- MOP 导入模板新增可选「达人」字段，可精准指定达人文件夹；未指定时仍按达人文件夹顺序自动匹配。
- 补充重复商家编码、达人字段选择和搜推 / KOL 两条链路的回归测试。

## 历史更新（2026-06-11 / v1.4.26）

- 桌面端版本升级到 `v1.4.26`，正式发布会输出 `crawshrimp-v1.4.26-*` 命名的桌面安装包。
- 修复森马云盘「天猫素材批量爬取-搭配购」在 Windows 后处理阶段偶发 `[WinError 2] 系统找不到指定的文件` 的问题。
- 搭配购素材打包改为先复制复用素材到包目录，再统一清理临时文件，避免同一下载源被多条对应 ID 记录复用时提前移动。
- 补充重复本地下载源路径的回归测试，确保导出的 ZIP 和 Excel 仍按原规则进入导出目录。
...
- `v1.4.28`：Temu 新增「商城-单款商品评价」任务，支持多链接批量抓取，并补齐评价国家、规格、合身情况、图片和头像等弹窗字段。
- `v1.4.27`：MOP 两个素材任务支持同款多达人图片包拆条、重复商家编码按行分配达人包，并新增可选「达人」模板字段。
