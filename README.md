...
## v2.4.4 Seedance 视频网关、巴拉视频模板与唯品会运营自动化（2026-08-04）

`v2.4.4` 汇总 `v2.4.3` 之后本地领先 GitHub 的 12 个提交：接入 Semir Seedance 网关，升级巴拉视频文案/上传模板和 AI 视频工作流，并新增唯品会热卖跟踪报表与包装、主图打标上传工具。

- AI 生视频支持 Semir Seedance 网关与 Ark 兼容 Base URL 归一化；Seedance CLI 增加别名配置、示例任务和回归覆盖，本地服务也能识别新的网关配置。
- AI 视频工作流继续收敛找图、AI 改图与生视频选择流；巴拉视频文案默认模型切到已验证配置，减少生成和打包参数错配。
- 巴拉短视频文案模板升级为 XLSX，视频标题拆分为「逛逛标题」和「搜推标题」，批量上传会填充带 `topicId` 的逛逛活动话题，并加入标题长度与重试校验。
- 唯品会新增热卖跟踪报表，按已登录页面读取五项运营报表，保留字段返回状态；缺失数据按 `未返回` / `不可算` 呈现，避免把缺失误报成 0 或 ROI。
- 唯品会包装+主图打标上传流程支持 PDC 预检、三档上传范围、包装展示图槽位、主图/列表图来源隔离、图片压缩和提交读回提示。
- 任务结果抽屉与进度摘要补充唯品会上传节点、预检/成功/警告状态，方便运营判断下一步是补权限、换素材还是继续上传。

完整变更见 [v2.4.4 Release Notes](release-notes/v2.4.4.md)。

## v2.4.3 TEMU 洗水唛与深绘鞋品图包修复（2026-07-31）

`v2.4.3` 汇总 `v2.4.2` 之后本地领先 GitHub 的 5 个提交：补齐 TEMU 洗水唛 SCM 洗护符号映射与固定符号兜底，继续加固深绘鞋品图包选择规则，并让 AI 视频工作流可以手动选择待审核素材进入视频任务。

- TEMU 洗水唛制作会优先从 SCM 洗护说明映射 TEMU 五项洗护符号；SCM 登录、查询或映射异常时使用 30℃水洗、不可漂白、阴干、110℃无蒸汽熨烫、不可干洗的固定符号继续处理。
- 洗水唛流程按最新业务反馈改为只记录 SCM 成分、备注和附件证据，不再把成分回填 TEMU；结果表补充洗护符号来源、说明和 SCM 洗护文本。
- 深绘鞋品图包会过滤 `.DS_Store`、`__MACOSX`、资源叉等系统隐藏文件，多尺码素材优先按 YK 标注选码，并保留必要的跨尺码 AI 角度图候选。
- 雪地靴鞋口/内里姿势和休闲鞋第 5 姿势规则进一步收敛；雪地靴 `tmz4/wpz4` 输出会把边缘连通背景重绘为统一灰底。
- AI 视频工作流的生视频阶段默认仍优先勾选已审核素材，但允许运营手动选择待审核的模特图、细节图和 AI 图。

完整变更见 [v2.4.3 Release Notes](release-notes/v2.4.3.md)。
