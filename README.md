...
## 上上次更新（2026-06-11 / v1.4.25）

- 桌面端版本升级到 `v1.4.25`，正式发布会输出 `crawshrimp-v1.4.25-*` 命名的桌面安装包。
- 新增 `mop-ops-assistant` 适配包，支持千牛素材中心批量 KOL 素材转短视频和搜推图文素材发布。
- MOP 两个任务都支持按商品 ID 或商家编码执行，提供 Excel / CSV 模板、字段说明、预检模式和 live 提交模式。
- MOP 素材输入支持业务图包目录扫描和手动多选图片，商家编码 KOC 素材包会在运行前展开为递归文件清单。
- 桌面端后端启动链路加固，Windows 上优先按模块方式启动 Python API，并在旧端口被占用时自动切换可用端口。
- 任务页会在执行前刷新目录文件扫描结果，减少先选目录后增删素材导致任务读取旧文件清单的问题。

## 历史更新（2026-06-10 / v1.4.24）

- 桌面端版本升级到 `v1.4.24`，正式发布会输出 `crawshrimp-v1.4.24-*` 命名的桌面安装包。
- 森马云盘「天猫素材批量爬取-搭配购」新增 SPU / SKC 抓取维度开关，支持按“款号-款色”精准检索单个款色文件夹。
- 搭配购导入模板新增“款色”列，SKC 模式下会自动拼接检索编码，并兼容 Excel 将 5 位色号读成数字的场景。
- 搭配购素材包收尾优化，打包阶段减少重复复制、图片 ZIP 改为无压缩存储，并避免导出目录额外生成同名素材文件夹。
- 补充搭配购 SKC 维度抓取、导出字段和素材包打包优化相关回归测试。
...
- `v1.4.26`：修复森马天猫搭配购素材包后处理在 Windows 上复用同一下载源文件时可能报 `[WinError 2]` 的问题。
- `v1.4.25`：新增 MOP 运营助手，支持 KOL 素材转短视频、搜推素材发布和商家编码素材包扫描，并加固 Windows 后端启动。
