...
## v2.4.2 深绘鞋品图包验收修复（2026-07-30）

`v2.4.2` 完成深绘鞋品上新图包的真实客户端回归修复，覆盖主图姿势、跨色匹配、细节图命名、渠道素材和导出结果表路径，并把服装与鞋品整理任务统一为默认打开全新页面。

- 主推色的 `1.jpg..N.jpg` 与已有 `yk1..ykN` 会按实际数量输出为 `yk*`，保留 `yk5` 及后续细节图，非主推色不输出 YK。
- 主推色细节图仍保留为姿势候选，视觉符合时可以同时复用于 `tmz/wpz/yq`，生成 YK 不限制其他槽位用途。
- 雪地靴主图 1、公主鞋主图 5、运动鞋底图及 O 图复用规则完成真实款号校验；唯品会素材保持 `wpz (1)` 到 `wpz (4)`、`wpz (15)`、`wpz (16)`。
- `jdt` 与 `wpt30` 只保留原始 PNG 渠道图；最终 Excel 会在图包移动到导出目录后刷新“本地文件”路径。
- 4 个款号真实重跑输出 472 行，其中 467 行下载成功且本地路径全部存在，4 张鞋盒标签图的款号红框均通过目视验收。

完整变更见 [v2.4.2 Release Notes](release-notes/v2.4.2.md)。

## v2.4.1 AI 视频工作流与深绘鞋品修复（2026-07-29）

`v2.4.1` 是 `v2.4.0` 之后的补丁发布，汇总本地领先 GitHub 的 3 个提交：优化 AI 视频工作流的改图体验和同步大图边界，并修复深绘鞋品上传包整理里的鞋盒标签打标、主图与细节图命名规则。

- AI 视频工作流的改图步骤支持更明确的素材边界、同步大图修改和界面状态回写，减少审核大图与实际改图素材不一致的问题。
- 巴拉 AI 视频助手的工作流参数、素材选择抽屉和后端打包逻辑补齐回归覆盖，保证改图任务只读取当前授权素材。
- 深绘鞋品上传包整理新增项目级 Tesseract OCR 服务，鞋盒标签打标会先识别款号文本位置再绘制红框，不再依赖固定坐标。
- 鞋品整理规则同步本次验收反馈：默认主图姿势 1、只给主推色输出 `o.jpg` 和 `yk*`，细节图按实际张数保留到 `yk5`，唯品会后两张命名为 `wpz (15)` 和 `wpz (16)`。

完整变更见 [v2.4.1 Release Notes](release-notes/v2.4.1.md)。

## v2.4.0 TEMU 洗水唛与 1XM 图片代理（2026-07-29）

`v2.4.0` 汇总 `v2.3.1` 之后本地领先 GitHub 的提交：新增 TEMU 洗水唛制作/官方下载自动化，并把 1XM 图片模型默认链路切换到抓虾 Cloudflare 代理，降低本地直连超时对 AI 生图任务的影响。

- TEMU 适配器新增「后台-洗水唛制作并下载官方 PDF」和「后台-洗水唛官方 PDF 批量下载」，支持按洗唛需求 Excel 去重 SKC、dry-run 预检、显式授权保存、回读已制作状态和官方 PDF 下载校验。
- 浏览器下载与 CDP 执行底座加强了新窗口、新标签和页面下载的等待/回收能力，补齐 `web_operator`、`browser_executor`、`runtime_downloads` 与 `core/js_runner.py` 的回归覆盖。
- 新增 Crawshrimp 1XM Cloudflare Proxy Worker；本地 1XM 图片客户端、设置页默认值和旧配置迁移统一使用 `https://one-xm-proxy.crawshrimp.com/v1`。
- 1XM 代理会重写任务轮询地址和图片结果地址，保留本地密钥配置路径，同时为后续 Worker 托管密钥模式预留部署说明和测试。

完整变更见 [v2.4.0 Release Notes](release-notes/v2.4.0.md)。

## v2.3.1 浅色主题与工作区恢复修复（2026-07-25）

`v2.3.1` 修复浅色模式下品牌按钮和 AI 视频工作流的主题不一致问题，并让已经移动或删除的旧工作区可以安全恢复。

- 浅色模式的品牌操作按钮统一为淘宝橙底、白色文字，覆盖 AI 生图、AI 视频工作流等主要操作入口。
- AI 视频工作流的审核页底部操作区和结果卡片信息区改用主题语义色；AI 生图失败态、AI 生视频加载/状态层、提示词警告态等隐藏状态也补齐浅色对比度。
- 工作区恢复错误改为简洁提示，原始技术信息收进“查看详情”，不会再覆盖顶部操作按钮。
- 检测到旧工作区目录不存在（`ENOENT`）时自动清除失效路径并要求重新选择目录，不会循环保存失败，也不会擅自创建空目录。

完整变更见 [v2.3.1 Release Notes](release-notes/v2.3.1.md)。
...
- 「AI 视频工作流」提供找图、AI 改图、审核、生视频、结果五个固定步骤，按款号组织原图与 AI 图，只把审核通过的素材交给视频任务。
