...
## 上上次更新（2026-05-08 / v1.4.10）

- 桌面端版本升级到 `v1.4.10`，正式发布会输出 `crawshrimp-v1.4.10-*` 命名的桌面安装包。
- 修复 Windows 桌面端首次安装或冷启动时，前端偶发 `connect ECONNREFUSED 127.0.0.1:18765` 的核心服务连接报错。
- Electron 主进程所有 FastAPI 调用统一走后端 ready / retry 控制，请求阶段短暂断连时会重新确认并尝试恢复后端。
- 后端彻底不可用时，错误信息会带上桌面日志路径、Python 路径和资源目录，方便排查真实启动原因。
- 桌面打包阶段如果缺少内置 Python bundle 会直接失败，避免产出安装后无法启动核心服务的 Windows 包。
...
- `v1.4.10`：补强 Windows 桌面端后端启动恢复、失败诊断和 Python bundle 打包校验。
