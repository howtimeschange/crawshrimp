...
## v2.4.6 Bala 视频提交与唯品会 OCR/R2 发布补丁（2026-08-05）

`v2.4.6` 是 `v2.4.5` 之后的补丁发布，聚焦 Bala AI 视频提交超时恢复、唯品会 OCR runtime 完整性，以及桌面更新镜像的 R2 大文件上传稳定性。

- Bala AI 视频「千鸟图生视频」提交链路恢复对长耗时接口的等待和重试，避免真实提交在任务创建阶段过早超时。
- 唯品会适配器恢复完整 Tesseract OCR runtime，随包携带 worker、core 和中英文语言包，异机只安装唯品会脚本也能完成详情图 OCR 定位。
- GitHub Actions 的 Cloudflare R2 更新镜像上传从 `wrangler r2 object put` 切到 S3 endpoint 上传，支持大文件 multipart，降低正式 tag 发布时 ZIP 超限失败的风险。

完整变更见 [v2.4.6 Release Notes](release-notes/v2.4.6.md)。
