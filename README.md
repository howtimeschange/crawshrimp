...
## 本次更新（2026-06-03 / v1.4.20）

- 桌面端版本升级到 `v1.4.20`，正式发布会输出 `crawshrimp-v1.4.20-*` 命名的桌面安装包。
- 新增 TikTok、速卖通、Shopee、Lazada、Shopify 多平台经营数据同步任务，并接入统一 ODPS 同步链路。
- 数据工作台任务页新增「运行并同步」入口，支持导出完成后继续执行数据同步。
- 深绘上新图包上传支持选择图片来源，便于按不同素材目录整理深绘上传包。
- Temu 后台鉴权与导航链路加固，多个任务在登录态、站点跳转和后台路由变化时更稳定。

## 上次更新（2026-06-01 / v1.4.19）

- 桌面端版本升级到 `v1.4.19`，正式发布会输出 `crawshrimp-v1.4.19-*` 命名的桌面安装包。
- 深绘上新图包整理默认不再自动打 ZIP，只按款号整理成文件夹输出到指定目录，降低最后打包阶段卡死风险。
- 深绘上新图包新增「是否自动压缩」开关，只有勾选后才会额外生成按款号划分的 ZIP 包，方便需要直接上传深绘时使用。
- 深绘模特图包按 SOP 跳过文件名以 `m` / `M` 开头的图片，不再下载或进入最终图包。
- 桌面端输出列表兼容文件夹结果，输出项会按 Excel、ZIP、文件夹分别显示更贴近真实产物的图标。

## 上上次更新（2026-05-28 / v1.4.18）

- 桌面端版本升级到 `v1.4.18`，正式发布会输出 `crawshrimp-v1.4.18-*` 命名的桌面安装包。
- 新增 `aliexpress-ops-assistant` 速卖通运营助手，首个场景为「商品抠图下载」。
- 商品抠图下载支持 Excel / CSV 款号模板上传、少量款号文本补充、去重、起止数据行分段重跑。
- 脚本按商品发布页第一张主图调用速卖通抠图接口并导出原图 / 抠图后图片地址，不点击发布页提交。
- 针对 1000+ 款批量任务增加接口退避重试、页面恢复重试、定期刷新、批量冷却和失败不中断逻辑。
- UI 为速卖通抠图任务接入增强进度条，显示当前款号、源表行号、成功 / 失败 / 重试摘要。
...
- `v1.4.20`：新增 TikTok、速卖通、Shopee、Lazada、Shopify 多平台经营数据同步，并加固 Temu 后台鉴权导航。
- `v1.4.19`：深绘上新图包默认改为按款号文件夹输出，新增可选自动压缩开关，并跳过模特图包中 `m` / `M` 开头图片。
- `v1.4.18`：新增速卖通运营助手，支持按 Excel / CSV 款号模板批量主图抠图下载，并接入增强进度条和大批量重试恢复。
