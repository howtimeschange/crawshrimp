...
## v2.5.2 上新运营助手、深绘服饰标签与鞋品语义验收补丁（2026-08-29）

`v2.5.2` 是 `v2.5.1` 之后的补丁发布，集中同步本地领先的上新运营助手适配包、深绘服饰吊牌/洗唛 AI 识别裁图、深绘鞋品语义共识验收、鞋品标签下载和买家秀目录保留改进。运营可以在抓虾内完成模拍/静物分图审核、审核后云盘上传和 PLM 尺码表下载；深绘服饰与鞋品图包在标签识别、槽位判断和真实款号验收上更稳。

- 新增「上新运营助手」适配包，支持模拍分图审核、审核后上传云盘、静物核表分图、静物核表上传和 PLM 尺码表 PDF 下载。
- 深绘服饰整理上新图包支持 YQ1 吊牌、YQ2 洗唛的 AI 识别与 800x800 白底裁图，并通过废图、款号、款色、尺码和 OCR 校验控制误选。
- 深绘鞋品图包引入结构化语义事实、多模型共识、槽位硬门禁和串行 rerun validator，真实款号验收会输出模型投票与问题证据。
- 鞋品标签瓦片下载、PDF/截图处理和后端 API 返回结构更稳定，减少鞋盒、吊牌、电子标签等素材在下载阶段丢失。
- 买家秀云盘生成任务保留模型图片来源的子文件夹结构，输出包更适合按模型、款式和交付目录复核。
- GLM 官方模型路由、LLM 设置和 OCR 服务补强，支撑深绘识别、服饰标签裁图和鞋品报告等多模型场景。

完整变更见 [v2.5.2 Release Notes](release-notes/v2.5.2.md)。
