...
## v2.4.9 TEMU AI 洗唛制作与运营稳定性补丁（2026-08-11）

`v2.4.9` 是 `v2.4.8` 之后的补丁发布，聚焦 TEMU AI 洗唛批量制作、SCM 洗护说明识别、官方 PDF 导出和失败重跑，同时补强 Bala/Vipshop 运营流程。

- 新增 TEMU「AI洗唛制作」任务：上传洗唛需求 Excel 后按款号展开全部 SKU，读取当前店铺、企业码和商品条码信息，按模板参数制作洗水唛并导出带尺码的官方 PDF。
- 接入 SCM 洗唛批复判定：优先使用 SCM 备注映射 TEMU 洗护符号，必要时下载 SCM 洗唛附件并调用多模态识别提取洗护说明；异常时继续使用固定 SOP 符号兜底。
- 洗唛制作保留预检/保存边界，保存后回读“已制作”与模板字段，校验 PDF 签名、文件大小和页面 API，并在结果表记录店铺、SKU、SCM 证据、字段差异、自动重试次数和失败原因。
- 补强 Excel 日期解析、字段回读修正、安全打印区自动加长、导出弹窗/编辑抽屉清理、已有 PDF 断点跳过和失败 SKU 自动重试，降低批量制作被单个异常打断的概率。
- 任务运行底座支持 AI 洗唛双阶段进度（展开款号/制作 SKU）、结果导出元数据和页面导航/下载重试，任务页可显示更清晰的实时进度。
- Bala AI 视频工作流放开本地图片/视频素材路径授权，清理素材时能安全删除 AI 结果缓存；唯品会包装+主图替换保留 PDC 查询兜底和更准确的失败汇总。

完整变更见 [v2.4.9 Release Notes](release-notes/v2.4.9.md)。

## v2.4.8 CDP 恢复与 Bala/唯品会流程稳定性补丁（2026-08-10）

`v2.4.8` 是 `v2.4.7` 之后的补丁发布，聚焦短视频 CDP 恢复、Bala AI 视频工作流素材/结果回收、唯品会包装+主图替换进度汇总，以及项目许可证声明。

- CDP 底座对 DevTools 管理端点超时、连接重置和半截 JSON 增加重试，并在页面 WebSocket 仍可用时保留当前连接，降低短视频任务因临时 CDP 抖动被整批打断的概率。
- 短视频批量任务在异常中断时会保留已完成、预检失败和中断行的结果，避免 fatal failure 后导出空表。
- Bala AI 视频工作流支持清空后的完整素材召回、来源素材选择保持、迟到 AI 改图结果刷新，以及失败视频任务的重跑路径保留。
- QN 图生视频和素材准备链路增强超时/结果恢复，补充人脸换背景、素材准备、审核和任务运行 API 的回归覆盖。
- 唯品会包装+主图替换脚本优化任务进度、执行汇总和结果读回，让运营更容易判断每个款号的找图、替换和失败原因。
- 项目自有代码许可证声明切换为 Apache-2.0，补充根目录 `LICENSE` / `NOTICE` 与应用包元数据。

完整变更见 [v2.4.8 Release Notes](release-notes/v2.4.8.md)。

## v2.4.7 SCM 洗唛吊牌下载与 Bala 视频队列补丁（2026-08-05）

`v2.4.7` 是 `v2.4.6` 之后的补丁发布，聚焦 SCM 洗唛吊牌批量下载、Bala 千鸟图生视频队列同步，以及短视频批量上传的防串款保护。

- 新增「SCM 运营助手」批量下载洗唛吊牌脚本，支持按款号去重查询 SCM 洗唛批复判定页，按款号整理洗唛/吊牌文件，并导出成分汇总、下载明细和异常表。
- SCM 洗唛吊牌参数面板保留运营常用输入，查询间隔和下载并发改为稳定默认值，减少执行前配置负担。
- Bala AI 视频「千鸟图生视频」批量任务补充排队、等待、运行中状态识别，避免同一任务在队列中被重复提交。
- 千鸟图生视频轮询接口暂时没有返回视频地址时，会从当前页面可见结果中读回视频链接，降低接口延迟导致的误判失败。
- Bala 短视频批量上传在真实执行时会跳过预检失败行，进入光合/搜推发布器会按款刷新页面，并阻止把上一款返回的内容 ID 写成本款成功结果。

完整变更见 [v2.4.7 Release Notes](release-notes/v2.4.7.md)。
