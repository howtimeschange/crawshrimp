...
## 本次更新（2026-04-27 / v1.4.4）

- 桌面端版本升级到 `v1.4.4`，`desktop-latest` 会输出 `crawshrimp-v1.4.4-*` 命名的桌面安装包。
- 新增 `shenhui-new-arrival` 深绘上新助手，覆盖森马云盘模特图 / 静物图整理、PDF 吊牌水洗截图、深绘图片包上传等上新链路。
- 深绘 `整理深绘上新图包` 支持选择上一轮失败结果表，只重跑下载失败 / 获取链接失败的云盘素材，并按款分批下载后继续汇总导出。
- 下载执行器补强断点复用、无代理直连、累计下载进度、当前文件、速度和字节数回传，前端进度条可展示更细的深绘下载状态。
- 启动时会清理孤儿任务运行态，避免桌面端异常退出后旧任务状态卡住后续运行。

## 上次更新（2026-04-24 / v1.4.3）

- 桌面端版本升级到 `v1.4.3`，`desktop-latest` 会输出 `crawshrimp-v1.4.3-*` 命名的桌面安装包。
...
## 上上次更新（2026-04-23 / v1.4.2）

- 桌面端版本升级到 `v1.4.2`，`desktop-latest` 会输出 `crawshrimp-v1.4.2-*` 命名的桌面安装包。
- 新增 `semir-cloud-drive` 森马云盘适配器，支持按配置目录批量检索款号/款色编码图片、下载整理、导出 Excel，并可选导出素材 zip。
- 森马云盘新增 `下载素材并自动生图` 任务：支持读取 Excel 模板、先批量下载素材，再跳转豆包 / Gemini 逐张上传参考图执行生图，并把 AI 结果图下载回本地打包。
...
| `semir-cloud-drive` | 森马云盘 / 豆包 / Gemini | 款图批量下载打包、按 Excel 批量下载素材并自动生图 |
...
- `v1.4.4`：新增深绘上新助手，支持失败结果表重跑、分批下载进度和深绘图片包上传链路。
- `v1.4.3`：Windows 打包版后端启动更稳，森马云盘新增款号代表图模式，并补强对应回归测试。
- `v1.4.2`：新增森马云盘批量下图和 AI 生图工作流，支持素材包导出、结果图回收和双阶段进度。
