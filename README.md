...
- SHEIN `商品分析-商品明细` 继续补强：导出字段已对齐页面指标配置，任务面板支持 `站点 / SKC / SPU / 在售状态 / 在架状态 / 上架日期 / 新品标签 / 商品层次 / 质量等级`，并补回执行进度条与探测失败静默回退。
...
- SHEIN `商品分析-商品明细` 已补齐页面筛选项继承、SKC/SPU 批量输入、下拉多选筛选与执行进度条恢复。
