...
## 本次更新（2026-06-15 / v1.4.31）

- 桌面端版本升级到 `v1.4.31`，正式发布会输出 `crawshrimp-v1.4.31-*` 命名的桌面安装包。
- Temu「商城-单款商品评价」增强错误诊断：脚本异常不再出现空白 `ERROR/FATAL`，会带出阶段、商品 ID、商品链接和当前页面等上下文，方便定位真实失败原因。
- Temu 单款商品评论接口请求失败时会按接口繁忙处理，继续走评论弹窗接口 / DOM 兜底链路，减少临时网络或页面接口异常导致的中断。
- Temu「商城-单款商品评价」Excel 导出文件名改为 `单款商品评价_{商品ID}_{时间}.xlsx`，不再把很长的商品标题放在文件名前缀里。
- JS 执行器遇到脚本返回失败但没有错误详情时，会统一显示 `脚本执行失败：未返回错误详情`，避免桌面端只看到空错误。

## 上次更新（2026-06-13 / v1.4.30）

- 桌面端版本升级到 `v1.4.30`，正式发布会输出 `crawshrimp-v1.4.30-*` 命名的桌面安装包。
- GitHub Actions 新增 pull request 构建入口，并收窄默认 token 权限；正式 Release 任务只在需要发布时申请写权限。
- 后端安全面收窄：manifest 通知条件移除 `eval()` 执行面，`/files/delete` 只允许删除已登记的运行时输出文件。
- 打包下载内置 Python runtime 时增加 SHA256 校验，避免仅靠 tar 解包校验判断下载可信。
- CDP / 下载 / 通知等阻塞路径改为异步友好执行，任务取消后会重新抛出 `CancelledError`，并等待部分导出收尾完成。
- 桌面端退出前会检测真实运行中的任务；用户确认后请求停止任务、等待收尾，再关闭后端和受管 Chrome，避免误杀用户自己的 Chrome。

## 上上次更新（2026-06-12 / v1.4.29）

- 桌面端版本升级到 `v1.4.29`，正式发布会输出 `crawshrimp-v1.4.29-*` 命名的桌面安装包。
- 桌面端启动 Python 后端前会先准备并写入探针验证真实可写的运行时数据目录，避免目录存在但不可写时后端启动失败。
- Windows 默认优先使用 `%LOCALAPPDATA%\crawshrimp`，macOS 默认优先使用 `~/Library/Application Support/crawshrimp`；如果检测到旧 `~/.crawshrimp` 里已有数据，会继续优先保留旧目录，避免升级后适配包、数据库、日志和导出缓存丢失。
- 后端与桌面端统一使用已解析的 `CRAWSHRIMP_DATA`，适配包、adapter-meta、data、logs、knowledge、api-token 和 backend.lock 会落在同一个运行时根目录。
- 桌面端每次启动都会给 Python 后端分配独立实例 ID，并通过 `/health.runtime` 校验连接到的确实是本次启动的后端；遇到同数据目录下旧后端或旧锁文件时会主动清理并重启，减少安装后界面打开但后端不可用的问题。
- macOS Dock 重新打开应用时会重新拉起窗口和后端服务，避免关闭窗口后再次点开只恢复界面、不恢复后台进程。

## 更早更新（2026-06-12 / v1.4.28）

- 桌面端版本升级到 `v1.4.28`，正式发布会输出 `crawshrimp-v1.4.28-*` 命名的桌面安装包。
- Temu 运营助手新增「商城-单款商品评价」任务，支持输入单个或多个 Temu 商城商品详情链接批量导出评论。
- 评论抓取优先复用页面评论接口；当接口繁忙或页面状态受限时，会打开商品页评论弹窗并从弹窗 DOM 兜底采集。
- 导出字段补齐评价国家、评价时间原文、规格、合身情况、评价图片、头像、有帮助人数和购买次数等弹窗内可见信息。
- 任务在 Temu 脚本列表中置顶，并修复脚本目录刷新后详情页排序没有同步的问题。
...
| `mop-ops-assistant` | 千牛素材中心 / MOP | KOL 素材批量转短视频、搜推图文素材批量发布、商家编码 KOC 素材包匹配 |
...
| `tmall-ops-assistant` | 天猫商品详情页 | 按多个商品链接批量抓取买家评价、追评、图片与规格信息 |

> 当前仓库内只保留一条 Shopee 适配线：`shopee-plus-v2`。旧 `shopee` / `shopee-plus` 已移除。

### 最近更新

- `v1.4.31`：Temu「商城-单款商品评价」补齐空错误诊断、接口失败兜底，并将 Excel 文件名简化为 `单款商品评价_{商品ID}_{时间}.xlsx`。
- `v1.4.30`：补齐桌面端安全加固、退出确认、受管 Chrome 生命周期治理、Python runtime SHA256 校验和 CI PR 构建。
- `v1.4.29`：桌面端启动链路加固，启动 Python 前验证真实可写运行目录，并按平台优先使用用户级应用数据目录，同时保留已有 `~/.crawshrimp` 数据。
