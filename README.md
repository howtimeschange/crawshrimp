...
## v2.5.1 桌面更新确认、Bala 质感控制与深绘鞋品识别补丁（2026-08-25）

`v2.5.1` 是 `v2.5.0` 之后的补丁发布，集中同步本地领先的桌面更新体验、Bala AI 视频素材质感控制、深绘鞋品图包识别和 Kimi K3 模型配置改进。运营在安装新版本前可以先查看更新说明，视频换脸素材可以按模型能力选择更高质量档位，深绘鞋品图包在复杂鞋型、姿势和槽位识别上更稳定。

- 桌面端应用内更新新增更新说明确认弹窗，下载完成后先展示版本变更、安装动作和重启提示，再由用户确认安装。
- 更新服务补强 release note/changelog 读取、延后安装和安装协调流程，避免后台下载完成后提示不清晰或重复触发。
- Bala AI 视频/换脸素材流程新增质量档位控制，前端会按模型能力展示可选质量，后端记录生成证据并保持任务结果可复核。
- 历史 AI 视频素材选择逻辑避免自动勾选已归档 AI 产物，只把当前流程内可用素材纳入默认选择，减少误用旧图。
- 深绘鞋品图包新增鞋品槽位规则与 rerun 校验工具，强化主图、角度图、鞋型姿势和缺失素材识别，失败原因更适合批量复核。
- LLM 配置补齐 `kimi-k3` 选项，Bala 视频文案、深绘识别等多模型入口可直接选择 Kimi K3。

完整变更见 [v2.5.1 Release Notes](release-notes/v2.5.1.md)。

## v2.5.0 AI 买家秀、深绘鞋品与多模型生产工作流（2026-08-22）

`v2.5.0` 是 `v2.4.16` 之后的功能级发布，新增 AI 买家秀工作流，并把本地领先的买家秀、深绘鞋品、AI 视频素材、Vipshop 图片替换和多模型配置改进合并到正式桌面版本。运营可以从买家秀需求 Excel 出发，自动下载森马云盘模拍图、匹配平铺参考图、调用 1XM/GPT Image 系列生成买家秀，并在结果队列中续看、续跑和打包交付。

- 桌面端新增「AI 买家秀工作流」入口，支持买家秀 Excel、模拍图下载、平铺图匹配、模型选择、自定义 Prompt、结果下载与本地打包。
- 后端新增买家秀服务与结果历史队列，失败任务会记录具体行级原因，图片生成类任务不再污染普通脚本历史。
- AI 生图与 AI 视频素材流程补齐去重、选择恢复和模型设置，DeepSeek 官方、DeepSeek 网关、Gemini 与 GPT Image 系列配置更一致。
- 深绘鞋品上新图包强化姿势模板、主图/角度图选择、失败隔离和进度回写，缺少单款素材时不再拖垮整批。
- Vipshop 主图替换链路补强 PDC 持久化校验，已标记替换结果会在页面读回后记录证据。
- 桌面端更新应用图标，脚本列表新增搜索，并移除云端审批 shell 探测对桌面启动的干扰。

完整变更见 [v2.5.0 Release Notes](release-notes/v2.5.0.md)。

## v2.4.16 TEMU 洗唛、Gemini 视频文案与深绘压缩补丁（2026-08-19）

`v2.4.16` 是 `v2.4.15` 之后的补丁发布，聚焦三条已上线链路的稳定性与交付质量：TEMU 洗唛制作能从 SCM 明细组件识别 PDF 洗唛附件，AI 视频文案默认使用 Gemini 3.5 Flash 并兼容 Gemini 图片传输，深绘上新/鞋品图包按大小自动压缩并把压缩结果写回明细表。

- TEMU 洗唛制作在 SCM 明细页读取附件时，会从渲染组件数据里提取洗唛、水洗和 wash 分组，识别图片与 PDF 洗唛附件，而不是只依赖页面可见图片节点。
- SCM 明细 PDF 洗唛会保留附件来源、文件名、类型和下载校验；PDF 下载继续使用 `%PDF-` 签名检查，便于后续 AI/OCR 识别与结果复核。
- AI 视频文案的默认文本模型切换为 `gemini-3.5-flash`，生意管家文案生成入口、全局 LLM 配置和后端默认值保持一致。
- Gemini 系列走 OpenAI 兼容接口时会把远程商品图转成 base64 data URL 发送，非 Gemini 模型仍保留图片 URL 直传，减少海外模型读取远程图失败。
- 短视频批量上传的标题长度校验改为按可见字符统计，避免 emoji、组合字符等场景下的字数误判。
- 深绘「整理上新图包」和「鞋品上新图包」会在单图或单款图包超过阈值时进行高质量压缩，并在结果 Excel 的「压缩结果」列回写原因与处理结果。

完整变更见 [v2.4.16 Release Notes](release-notes/v2.4.16.md)。
...
- 从买家秀需求 Excel 批量读取款号、款色、尺码和行级需求，自动整理输出目录和结果工作簿。
- 通过已登录的森马云盘会话下载人物模拍图，并按款色号匹配商品平铺参考图。
- 支持 GPT Image/1XM 系列模型、默认买家秀换装 Prompt 与单批自定义 Prompt。
- 生成过程记录行级状态、失败原因、本地文件、模型任务 ID 和打包路径，便于复核与续跑。
- 结果历史独立于普通脚本任务，可在桌面端继续查看、打开目录、下载结果或清理记录。
