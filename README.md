...
## 上上次更新（2026-06-09 / v1.4.23）

- 桌面端版本升级到 `v1.4.23`，正式发布会输出 `crawshrimp-v1.4.23-*` 命名的桌面安装包。
- 修复 Windows 上深绘上新图包整理时 PDF 吊牌/洗唛截图失败的问题，缩短 PyMuPDF 中间渲染路径并降低中文长路径影响。
- 深绘 PDF 截图流程改为优先从内存流打开 PDF，中间页图使用 `p1.png` 等短文件名，减少 Windows 保存截图时报 `cannot open file` 的概率。
- 深绘上新图包与 PDF 批量截图任务共用更短的 `_pdf_work` 临时目录，最终图包命名规则保持不变。
- 补充深绘 PDF 渲染和图包整理短路径相关回归测试。
...
- `v1.4.24`：森马天猫搭配购素材支持 SPU / SKC 维度抓取，模板新增款色列，并优化素材包打包收尾。
- `v1.4.23`：修复 Windows 上深绘上新图包 PDF 截图中间路径过长导致 PyMuPDF 渲染失败的问题。
