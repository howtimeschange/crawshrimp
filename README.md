...
## v2.2.1 AI 视频工作流体验完善（2026-07-17）

`v2.2.1` 完善 AI 视频工作流的批量操作、素材审核与生视频工作台体验，并修复收藏轮询与视频重复提交等边界问题。

- 找图、AI 改图、审核、生视频页面统一提升信息密度、状态反馈、批量选择与图片卡片交互；审核支持批量通过和批量舍弃。
- 生视频任务支持按状态筛选、批量预检/提交，并阻止已生成或已下载的视频被重复提交；刷新后停留在可继续操作的步骤。
- AI 生视频工作台采用 3:4 竖屏卡片，支持列表内播放、明确的删除确认（仅删除记录或删除本地文件），并移除重复创建任务入口。
- 脚本收藏改为稳定的书签样式；修复取消收藏与轮询并发时的旧数据回写，收藏列表按添加时间顺序展示。

完整变更见 [v2.2.1 Release Notes](release-notes/v2.2.1.md)。
