...
## 上上次更新（2026-06-09 / v1.4.22）

- 桌面端版本升级到 `v1.4.22`，正式发布会输出 `crawshrimp-v1.4.22-*` 命名的桌面安装包。
- 桌面端任务列表启动链路加固，后端启动、适配包加载和任务生命周期接口异常时会给出更稳定的状态反馈。
- 深绘上新图包最终打包链路增强，文件夹收尾、PDF 截图回退和异常结果落盘更稳，降低批量整理尾段失败概率。
- 桌面构建内置 Python 依赖缓存刷新逻辑增强，命中缺失 `xlrd` 等运行依赖时会自动清理旧缓存并重新下载。
...
- `v1.4.22`：加固桌面后端任务列表、深绘上新图包最终打包和内置 Python 依赖缓存刷新链路。
