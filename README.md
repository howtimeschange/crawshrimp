...
## v2.3.0 电商视频、图包与桌面体验升级（2026-07-25）

`v2.3.0` 是一次功能级发布，汇总 `v2.2.2` 之后尚未打 tag 的全部提交：新增短视频文案生成与批量上传、SHEIN 图包下载、鞋品深绘上传包整理，并补齐浅色主题、文本大模型配置和按风格参考图工作流。

- 巴拉 AI 视频助手新增商品视频标题/文案生成与短视频批量上传：复用千牛登录态读取商品信息，可按 Excel 和本地视频批量发布逛逛、搜推素材并更新商品展示视频，保留接口回执与刷新读回。
- 森马云盘新增 SHEIN 图包下载；深绘上新助手新增鞋品上传包工作流，按款号识别平拍、模拍、吊牌和洗唛素材，保留原图并输出可追踪的结果表。
- 天猫 AI 测图支持每个款式独立选择自定义参考图，提示词操作与任务进度反馈进一步收敛；视频素材选择、缩略图恢复和 10MB 图片边界更加稳定。
- 桌面端新增浅色、深色与跟随系统主题；设置页统一管理文本大模型供应商、模型与连通性验证，为视频文案等任务提供复用能力。

完整变更见 [v2.3.0 Release Notes](release-notes/v2.3.0.md)。
