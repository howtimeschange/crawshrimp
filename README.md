...
- 按候选数量调整模型识别时限；全量图存在分歧时自动分页、放大候选并逐槽位复核，保持两个独立模型确认的选图要求。
- 对已有功能卡候选但未确认的 yx 增加专项复核，避免直接按可缺少项省略；单独鞋垫、卡片、鞋盒和局部特写仍不可充当 yx。
- 全部款式组包失败时明确显示失败及具体原因，保留已下载原素材和结果表，支持指定目录及默认输出目录。
- 同款两色实图逐张核验通过，确认 5 张 tmz 同色、12 张 wpz 和 2 张 yx；相关 307 项回归测试通过。

完整变更见 [v2.5.6 Release Notes](release-notes/v2.5.6.md)。
